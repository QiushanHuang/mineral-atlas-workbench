...
def report(model,quality,spec):
 ...
 body+='</table>'+appendix()+'<h2>校验与来源</h2><pre>'+e(json.dumps(quality,ensure_ascii=False,indent=2))+'</pre><pre>'+e(json.dumps(spec.get('evidence',[]),ensure_ascii=False,indent=2))+'</pre>'
...
def build_project(spec,output,attachment=None):
 ...
 with LOCK:
  ...
  try:
   ...
   write_json(tmp/'point-groups-32.json',CATALOGUE)
   (tmp/'point-groups.html').write_text(reference_page(),encoding='utf-8')
